...
class AgentConfig(BaseSettings):
    ...
    # How many chat lines around a detected URL to send to the server for context.
    context_lines: int = Field(10, description="Context lines sent with each classification request", ge=1)

    # Credentials generated by the dashboard when a child is registered.
    # Paste the value returned by 'Add child' into agent_config.json.
    agent_token: str = Field("", description="Per-child agent token issued by the parent dashboard")

    # One-time setup code embedded in the installer by the server.
    # The agent exchanges this for the real agent_token on first run via
    # POST /agent/activate, then clears this field.
    setup_code: str = Field("", description="One-time activation code (cleared after first run)")
