...
def _classify(url: str, context: str, detection_source: str) -> _ClassifyResult:
    ...
    try:
        ...
        log.debug(
            "Server classify → %s  confidence=%d%%  reason=%r  url=%s",
...
def _sender_loop() -> None:
    ...
    while True:
        ...
        try:
            ...
            resp.raise_for_status()
            log.debug("Hop event POSTed  (HTTP %d).", resp.status_code)
        except requests.exceptions.RequestException as exc:
...
def _poll_browser_title(
    ...
    while time.monotonic() < deadline:
        ...
        if domain in title.lower():
            log.warning(
                "CONFIRMED hop — browser navigated to %r  (title: %r)", domain, title
            )
            _enqueue_hop({**event, "clickConfidence": "title_match"})
            return
        time.sleep(interval)

    log.info("Browser title poll timed out — no domain match for %r.", domain)
    _enqueue_hop({**event, "clickConfidence": "switch_only"})
...
def _confirm_pending(
    ...
    if _app_matches_url(lure_url, proc):
        log.info("Click confidence: app_match  (%r matches lure %r)", proc, lure_url)
        _enqueue_hop({**base_event, "clickConfidence": "app_match"})

    elif proc in _BROWSER_PROCESSES:
        domain = _extract_domain(lure_url)
        log.info("Click confidence: polling browser title for domain %r …", domain)
        threading.Thread(
        ...
        log.info("Click confidence: switch_only.")
...
def _try_late_confirm(game_proc: str) -> None:
    ...
    if last and age <= _LATE_CONFIRM_MAX_AGE:
        log.warning(
...
def _decide_and_send(
    ...
    if not result.is_hop:
        log.info(
            "Classify: innocent  confidence=%d%%  reason=%r  via=%s.",
            result.confidence, result.reason, result.via,
        )
        return

    log.warning(
        "Hop attempt detected — %s  confidence=%d%%  reason=%r  via=%s.  "
        "Watching for app switch…",
        url, result.confidence, result.reason, result.via,
    )
...
def _scanner_loop(
    ...
    log.info("Scanner started for %r.", game_proc)
    seen_urls = _TTLCache(ttl_seconds=300)
    ...
    while not stop.is_set():
        ...
        for url in _find_urls(text):
            if seen_urls.seen(url):
                continue

            context = _extract_context(text, url)
            log.info("URL detected in %r: %s — classifying…", game_proc, url)
    ...
    log.info("Scanner stopped for %r.", game_proc)
...
def _clipboard_monitor_loop(stop: threading.Event) -> None:
    ...
    while not stop.is_set():
        ...
        game = _current_game
        if game is None:
            seen_urls = _TTLCache(ttl_seconds=300)  # reset so prior links re-evaluate next session
            last_content = ""
            stop.wait(1.0)
            continue
        ...
        if current != last_content:
            ...
            for url in _find_urls(current):
                if seen_urls.seen(url):
                    continue
                log.info("URL found in clipboard: %s — classifying…", url)
                threading.Thread(
                    target=_decide_and_send,
...
def _process_foreground_change(hwnd: int) -> None:
    ...
    log.info(
        "Foreground: %r → %r  (%r → %r)",
        _prev_proc, proc, _prev_title, title,
    )
    ...
    if proc not in _TRANSIT_PROCESSES:
        ...
        if snapshot:
            if _is_game(proc):
                ...
                for pending in snapshot.values():
                    log.warning(
                        "CONFIRMED hop after lure — child switched to %r.", proc
                    )
                    _confirm_pending(pending, proc, title, hwnd)
    else:
        log.debug("Transit process %r — keeping pending hop(s) alive.", proc)
...
def run() -> None:
    ...
    log.info(
        "HopMap agent started.  Current app: %r (%r).  Press Ctrl-C to stop.",
        _prev_proc, _prev_title,
    )
